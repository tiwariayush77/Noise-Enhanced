    ...
    <Card className="p-4 bg-card overflow-hidden relative">
      <div className="flex gap-4">
        ...
        <div className="flex-1 space-y-1">
          ...
          {action ? (
             <Button variant="link" className="p-0 h-auto text-accent">{action}</Button>
          ) : (
        ...
        {badge && (
          <Badge className="absolute top-2 right-2 bg-success text-black">{badge}</Badge>
        )}
      </div>
    </Card>
  );
      ...
      <Card className="bg-card rounded-xl p-6 text-center">
        ...
        <div className="relative inline-block">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                 <div className="text-2xl font-bold">{mockData.ecosystemScore}<span className="text-lg">/100</span></div>
            </div>
        </div>
      ...
      <div className="space-y-4">
        {devices.map(device => (
          <DeviceCard
            ...
            icon={device.type === 'watch' ? '⌚' : device.type === 'ring' ? '💍' : '🎧'}
